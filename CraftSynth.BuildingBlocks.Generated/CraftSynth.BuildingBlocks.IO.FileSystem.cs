...
public class FileSystem
{
	...
	public static string InsureProperDirectorySeparatorChar(string s, string nullCaseResult = null)
	{
		string r;
	...
	public static string PathCombine(string p1, string p2)
	{
		...
		{
			p1 = InsureProperDirectorySeparatorChar(p1);
			p2 = InsureProperDirectorySeparatorChar(p2);
			r = p1.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p2.TrimStart(Path.DirectorySeparatorChar);
		}
		return r;
	}
	...
	public static string PathCombine(string p1, string p2, string p3)
	{
		...
		else
		{
			p1 = InsureProperDirectorySeparatorChar(p1);
			p2 = InsureProperDirectorySeparatorChar(p2);
			p3 = InsureProperDirectorySeparatorChar(p3);
			r = p1.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p2.Trim(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p3.TrimStart(Path.DirectorySeparatorChar);
		}
		return r;
	}

	public static string PathCombine(string p1, string p2, string p3, string p4)
	{
		string r;
		...
		else
		{
			...
			p2 = InsureProperDirectorySeparatorChar(p2);
			p3 = InsureProperDirectorySeparatorChar(p3);
			p4 = InsureProperDirectorySeparatorChar(p4);
			r = p1.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p2.Trim(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p3.Trim(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + p4.TrimStart(Path.DirectorySeparatorChar);
		}
		return r;
	}

	public static string GetAbsolutePath(string absoluteOrRelativePath, string nullCaseResult = null, bool insureProperDirectorySeparatorChar = true, bool preTrimNonPrintableChars = true)
	{
		string r;
		...
		{
			if(preTrimNonPrintableChars)
			{
				absoluteOrRelativePath = absoluteOrRelativePath.TrimNonPrintableChars();
			}

			if(absoluteOrRelativePath.StartsWith('\\') || absoluteOrRelativePath.StartsWith("/"))
			{
				absoluteOrRelativePath = PathCombine(CBB_CommonMisc.ApplicationRootFolderPath, absoluteOrRelativePath);
			}

			r = absoluteOrRelativePath;
		}

		if(r != null && insureProperDirectorySeparatorChar)
		{
			r = InsureProperDirectorySeparatorChar(r, nullCaseResult);
		}
	...
	public static bool FileOrFolderExistsUsingZetaForLongPaths(string path)
	{
		...
		var zfi = new ZlpFileInfo(path);
		r = zfi.Exists;

		if(r == false)
		{
			var zdi = new ZlpDirectoryInfo(path);
			r = zdi.Exists;
		}
	...
	/// <returns></returns>
	public static List<string> GetFilePaths(string folderPath, bool recursively = false, string searchPattern = null, bool ignoreUnauthorizedAccessExceptions = false)
	{
		...
		try
		{
			if(Directory.Exists(folderPath))
			{
				...
				if(recursively)
				{
					...
					{
						List<string> subfolderFiles = GetFilePaths(subfolder, recursively, searchPattern, ignoreUnauthorizedAccessExceptions);
	...
	private static List<string> GetFilePathsUsingZetaForLongPaths(string folderPath, bool recursively = false, string searchPattern = null)
	{
		...
		var zdi = new ZlpDirectoryInfo(folderPath);
		if(zdi.Exists)
		{
		...
		try
		{
			if(Directory.Exists(folderPath))
			{
				...
				foreach(string subfolder in subfolders)
				{
					...
					{
						List<KeyValuePair<string, bool>> subfolderItems = GetFileAndFolderPaths(subfolder, true, includeFilesInResult, includeFoldersInResult, recursively, searchPattern, true);
	...
	public static void WriteBytesToFile(string filePath, byte[] data)
	{
		...
		{
			throw new ArgumentException("Value must be non-empty string.", "filePath");
		}
	...
	private class FileToRecycleBinMover
	{
		...
		private static List<ErrorTableItem> ErrorTable
		{
			get
			{
				var items = new List<ErrorTableItem>();
	...
	{
		var di = new ZlpDirectoryInfo(path);
		// Delete all files and sub-folders?
		if(recursive)
		{
			// Yep... Let's do this
			ZlpDirectoryInfo[] subfolders = di.GetDirectories(path);
			foreach(ZlpDirectoryInfo? s in subfolders)
			{
		...
		// Get all files of the folder
		ZlpFileInfo[] files = di.GetFiles(path);
	...
	public static bool RemoveReadOnlyAttributeIfExistsUsingZetaLongPaths(string filePath)
	{
		...
		var fi = new ZlpFileInfo(filePath);
	...
	public static string GetAppSetting(IConfiguration configuration, string key)
	{
		...
		{
			throw new ArgumentException("Value must be non-empty string.", "key");
		}
	...
	/// <returns>Value linked to specified key. 'null' if key not found or error occured.</returns>
	/// <exception cref="ArgumentException">Thrown when parameter is null or empty.</exception>
	public static int? GetAppSettingAsInt(IConfiguration configuration, string key)
	{
		int? result;

		string value = GetAppSetting(configuration, key);
	...
	private static Dictionary<string, object> _appendFileLockObjects = new Dictionary<string, object>();
	public static void AppendFile(string filePath, string text, ConcurrencyProtectionMechanism concurrencyProtectionMechanism, string mutexName = null)
	{
		switch(concurrencyProtectionMechanism)
		{
			...
			case ConcurrencyProtectionMechanism.Lock:
				...
				lock(_appendFileLockObjects)
				{
					if(_appendFileLockObjects == null)
					{
						_appendFileLockObjects = new Dictionary<string, object>();
					}
					if(!_appendFileLockObjects.ContainsKey(filePath.ToUpper()))
					{
						_appendFileLockObjects.Add(filePath.ToUpper(), new object());
					}
					lockObject = _appendFileLockObjects[filePath.ToUpper()];
				}
			...
			case ConcurrencyProtectionMechanism.Mutex:
				...
				{
					mutexName = "CBB_AppendFileMutex-" + filePath.ToUpper().Replace('\\', '_').Replace('/', '_');
	...
	public static List<string> GetAllChildrensAndDescendants(string path, List<string> allPaths, bool includeItselfInResult, bool includeNonFirstLevelDescendants, bool performSimpleAlphanumericSortOnFinalList)
	{
		...
		path = path.ToLower().TrimEnd(Path.DirectorySeparatorChar);
		foreach(string itm in allPaths)
		{
			string item = itm.TrimEnd(Path.DirectorySeparatorChar);
			...
			{//add descendent if neccessary
				bool firstLevelDescendant = !item.Substring(path.Length + 1).Contains(Path.DirectorySeparatorChar.ToString());
	...
	public static T GetSettingFromIniFile<T>(string settingName, string filePath, bool throwErrorIfNotFound, T resultInNotFoundCase, bool throwErrorIfEmptyString, T resultInEmptyStringCase, bool hideErrors, T resultInErrorCase, char separatorBetweenKeyAndValue = '=')
	{
		...
		catch(Exception exception)
		{
			...
			{
				var outerException = new Exception(string.Format("Can not read setting '{0}' from file '{1}'.", settingName ?? "null", filePath ?? "null"), exception);
	...
	public static double GetTotalSpaceForDriveByFileOrFolder(string fileOrFolderPath, DiskSizeUnit unit)
	{
		...
		double space = GetTotalSpaceForDrive(drive, unit);
		return space;
	}
	...
	public static double GetFreeSpaceForDriveByFileOrFolder(string fileOrFolderPath, DiskSizeUnit unit)
	{
		string driveLetter = Path.GetPathRoot(fileOrFolderPath);
		var drive = new DriveInfo(driveLetter);
		double space = GetFreeSpaceForDrive(drive, unit);
		return space;
	}

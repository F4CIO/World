...
namespace CraftSynth.BuildingBlocks.IO.Http
{
	public class Misc
	{
		...
		public static string LastResponseUri = null;
        public static HttpStatusCode? LastResponseStatusCode = null;
		...
		public static string RequestUri(
			...
			int requestUriContentType = 0, 
			object customTraceLog = null, 
			bool blockAllExceptions = false, 
			string errorCaseResult=null, 
			bool expectCookies = false, 
			List<Cookie> cookies = null, 
			bool detailedLog = true, 
			System.Text.Encoding encoderForParsingResult = null, 
			string fileNameWhereToDumpResponse = null, 
			bool allowAutoRedirect = true, 
			bool repeatRequestWhileReceivingRedirectStatusCode = false,
			int? repeatRequestMaxCycles = 5,
            string proxySchemaDomainAndPort = null,
			...
			)
		{
			...
			{				
				...
				bool makeRequestOnceAgain = true;
				while (makeRequestOnceAgain)
				{
					...
					if (request is HttpWebRequest && !allowAutoRedirect)
					{
                    ...
                    if(proxySchemaDomainAndPort!=null)
                    {//source: https://stackoverflow.com/questions/16420552/web-request-error-407-proxy-authentication-required
                        if (username == null)
                        {
                            request.Credentials = new NetworkCredential(proxyUsername, proxyPassword);
                        }
                        WebProxy webProxy = new WebProxy(proxySchemaDomainAndPort, true)//"http://myproxy.net:8080/"
                        {
                    ...
                    LastResponseUri = response.ResponseUri.AbsoluteUri;
                    LastResponseStatusCode = response.StatusCode;

					bool receivedRedirectedStatusCode = response.StatusCode == HttpStatusCode.Moved || response.StatusCode == HttpStatusCode.Redirect || response.StatusCode == HttpStatusCode.MovedPermanently || response.StatusCode == HttpStatusCode.TemporaryRedirect || response.StatusCode == HttpStatusCode.TemporaryRedirect || response.StatusCode == HttpStatusCode.RedirectKeepVerb || ((int)response.StatusCode) == 308 /*The remote server returned an error: (308) Permanent Redirect*/;
					if (receivedRedirectedStatusCode && !allowAutoRedirect && repeatRequestWhileReceivingRedirectStatusCode)
					{
						//prepare another cycle
						repeatRequestMaxCycles--;
						makeRequestOnceAgain = repeatRequestMaxCycles > 0;
						if (makeRequestOnceAgain)
						{
